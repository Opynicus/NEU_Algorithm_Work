...
mpz_t p, g, k,kk,c1,c2;
mpz_t xa;
mpz_t xb;
mpz_t ya;
mpz_t yb;
mpz_t shared_keyAC;//相同的私钥
mpz_t ma, mb;
mpz_t yaa, ybb;
mpz_t c1a, c1b;
mpz_t c2a, c2b;
...
int init1()
{
	...
	char tmp4[1024];
	char tmp5[1024];
	ifstream in1("A_Elgamal.txt", ios::in);
	//判断文件是否正常打开
	if (!in1) {
		cout << "error" << endl;
		return 0;
	...
	in1.close();
	mpz_set_str(xa, tmp1, 37);
	mpz_set_str(ya, tmp2, 37);
...
int SendMsg2Third()
{
	ofstream out("A_Msg.txt", ios::out);
	if (!out)
	{
	...
	mpz_get_str(sharedkey, 37, shared_keyAC);
	mpz_get_str(pp, 37, p);
	mpz_get_str(gg, 37, g);
	int i = 0;
	while (i < strlen(c1a1))
...
int main()
{
	...
	PublicKeyGen(p, g, &shared_keyAC, &yaa, &kk);
	clock_t startTime, endTime;
	startTime = clock();
	Elgamal_Encryption(ma, ya, k, p, g, &c1a, &c2a);
	/*Elgamal_Decryption(c1a, c2a, xa, p, g, &Getc1a);
	gmp_printf("%Zd\n", ma);
	gmp_printf("%Zd",Getc1a);*/


	Elgamal_Encryption(c1a, yaa, kk, p, g, &c1a_1, &c1a_2);
	Elgamal_Encryption(c2a, yaa, kk, p, g, &c2a_1, &c2a_2);
	/*Elgamal_Decryption(c1a_1, c1a_2, shared_keyAC, p, g, &Getc1a);
	Elgamal_Decryption(c2a_1, c2a_2, shared_keyAC, p, g, &Getc2a);
	gmp_printf("%Zd\n", c1a);
	gmp_printf("%Zd\n", Getc1a);
	gmp_printf("%Zd\n", c2a);
	gmp_printf("%Zd\n", Getc2a);*/
