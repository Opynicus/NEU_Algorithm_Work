...
int C2A()
{
	...
	char c_1[1024];
	char c_2[1024];
	mpz_get_str(c_1, 37, c1a);
	mpz_get_str(c_2, 37, c2a);
	int i = 0;
	while (i < strlen(c_1))
	{
		out << c_1[i];
...
int C2B()
{
	...
	char c_3[1024];
	char c_4[1024];
	mpz_get_str(c_1, 37, c1a_1);
	mpz_get_str(c_2, 37, c1a_2);
	mpz_get_str(c_3, 37, c2a_1);
	mpz_get_str(c_4, 37, c2a_2);
	int i = 0;
	while (i < strlen(c_1))
	{
		out << c_1[i];
...
int main()
{
	...
	startTime1 = clock();
	Elgamal_Encryption(c1a, y, k, p, g, &c1a_1, &c1a_2);
	Elgamal_Encryption(c2a, y, k, p, g, &c2a_1, &c2a_2);
	endTime1 = clock();//计时结束
