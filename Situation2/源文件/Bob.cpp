...
int init1()
{
	...
	char tmp2[1024];
	char tmp3[1024];
	ifstream in1("DHSharedKeyBC.txt", ios::in);
	if (!in1) {
		cout << "error" << endl;
		return 0;
	}
	...
	in1.close();
	mpz_set_str(shared_KeyBC, tmp1, 37);
...
int init2()
{
	...
	in1.close();
	mpz_set_str(c1b_1, tmp1, 37);
	mpz_set_str(c1b_2, tmp2, 37);
	mpz_set_str(c2b_1, tmp3, 37);
	mpz_set_str(c2b_2, tmp4, 37);
	ifstream in3("IsCheating.txt", ios::in);
	if (!in3) {
		cout << "error" << endl;
		return 0;
	}
	in3.getline(judge_str, 1024);
	in3.close();
}
int init3()
{
...
int B2A()
{
	...
	char c_1[1024];
	char c_2[1024];
	mpz_get_str(c_1, 37, c1b);
	mpz_get_str(c_2, 37, c2b);
	int i = 0;
	while (i < strlen(c_1))
	{
		out << c_1[i];
...
int main()
{
	...
	startTime = clock();
	Elgamal_Encryption(mb, y, k, p, g, &c1b, &c2b);
	B2A();
	endTime = clock();//计时结束
	...
	startTime1 = clock();
	if (strcmp(judge_str, "yes") == 0)
	{
		cout << "Alice作弊，终止游戏" << endl;
		system("pause");
		return 0;
	}
	else
	...
	startTime2 = clock();
	if ((mpz_cmp(c1a_1, c1b_1) == 0) && (mpz_cmp(c2a_1, c2b_1) == 0)&& (mpz_cmp(c1a_2, c1b_2) == 0) && (mpz_cmp(c2a_2, c2b_2) == 0))
	{
		cout << "成绩一致" << endl;
	}
	else
	...
	endTime2 = clock();//计时结束
	cout << "该程序运行时间: " << (double)(endTime2 - startTime2) / CLOCKS_PER_SEC << "s" << endl;
	system("pause");
}